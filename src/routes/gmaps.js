...
gmaps.post("/", async (c) => {
  ...
  if (!Array.isArray(fields) || fields.length === 0) {
    return c.json(
      { success: false, error: '"fields" must be a non-empty array' },
      400,
    );
  }
  ...
  if (total < 1 || total > 100) {
    return c.json(
      { success: false, error: '"total" must be between 1 and 100' },
      400,
    );
  }

  console.log(
    `[API] New scrape request: query="${query}" total=${total} fields=[${fields.join(",")}]`,
  );
