...
} from "./gmap/helpers.js";
import { SELECTORS } from "./gmap/selectors.js";
...
async function extractBasicInfo(page, fields) {
  ...
  if (fields.includes("name")) {
    result.name = await page
      .$eval(SELECTORS.placeName, (el) => el.textContent?.trim())
      .catch(() => null);
  }
  ...
  if (fields.includes("category")) {
    result.category = await page
      .$eval(SELECTORS.placeCategory, (el) => el.textContent?.trim())
      .catch(() => null);
  }

  // Rating
  if (fields.includes("rating")) {
    result.rating = await page
      .$eval(SELECTORS.placeRating, (el) => el.textContent?.trim())
      .catch(() => null);
    ...
    result.priceLevel = await page
      .evaluate(() => {
        ...
        const el = document.querySelector(
          '[aria-label*="Harga:"], [aria-label*="Price:"]',
        );
        if (el)
          return el
            .getAttribute("aria-label")
            .replace(/Harga:|Price:/gi, "")
            .trim();
        ...
        for (const d of allDivs) {
          if (
            d.textContent &&
            d.textContent.includes(" per orang") &&
            (d.textContent.includes("Rp") || d.textContent.includes("IDR"))
          ) {
    ...
    result.hours = await page
      .evaluate(() => {
        ...
        for (const el of allDivs) {
          ...
          if (
            text.match(
              /(Buka|Tutup|Open|Closed)\s*[·⋅]\s*(Buka|Tutup|Open|Closed).*?(pukul|at|until)\s*\d/i,
            )
          ) {
            return text
              .split("")[0]
              .split("Jam buka")[0]
              .split("Tutup segera")[0]
              .replace(/^.*?Buka/, "Buka")
              .replace(/^.*?Tutup/, "Tutup")
              .replace(/^.*?Open/, "Open")
  ...
  if (fields.includes("reservation")) {
    result.reservation = await page
      .evaluate(() => {
        const links = Array.from(
          document.querySelectorAll('a.CsEnBe, a[data-item-id^="action:"]'),
        );
        for (const a of links) {
          if (
            a.textContent.includes("Reservasi") ||
            a.textContent.includes("Reservation") ||
            a.textContent.includes("Book") ||
            a.textContent.includes("Pesan")
          ) {
            return a.href;
          }
        }
...
async function extractHours(page) {
  try {
    ...
    const hoursBtn = await page.$(
      'button[jsaction*="openhours"], button[aria-label*="jam"], button[aria-label*="hours"]',
    );
    if (hoursBtn) {
...
async function extractReviews(page, maxReviews = 10) {
  try {
    ...
    const moreButtons = await page.$$(
      'button[aria-label*="Selengkapnya"], button[aria-label*="More"], button.w8nwRe',
    );
...
async function sortReviewsByNewest(page) {
  try {
    ...
    const sortBtn = await page.$(
      'button[aria-label*="Urutkan"], button[aria-label*="Sort reviews"], button[data-value*="sort"]',
    );
    if (!sortBtn) return;
    await sortBtn.click();
    await randomDelay(500, 1000);
    ...
    const newestOption = await page.$(
      'li[aria-label*="Terbaru"], li[data-index="1"], div[data-index="1"]',
    );
    if (newestOption) {
...
async function extractAboutFeatures(page, fields) {
  ...
  try {
    ...
    const aboutTabSelectors =
      'button:has-text("Tentang"), button:has-text("About"), div[role="tab"]:has-text("Tentang"), div[role="tab"]:has-text("About")';
    ...
    if (aboutTab) {
      ...
      console.log(
        "[Scraper] About tab NOT found with selectors:",
        aboutTabSelectors,
      );
      if (process.env.GMAPS_DEBUG === "1") {
        await page
          .screenshot({ path: `debug-about-tab-missing-${Date.now()}.png` })
          .catch(() => {});
      }
    }
      ...
      sectionEls.forEach((section) => {
        ...
        itemEls.forEach((item) => {
          ...
          if (label) {
            ...
            const isUnavailable =
              ariaLabel.toLowerCase().includes("tidak") ||
              ariaLabel.toLowerCase().includes("no ");
  ...
  try {
    ...
    const menuItems = await page.evaluate(() => {
      ...
      const menuEls = document.querySelectorAll(
        "div.WbznCd li.qjESne, ul.ZWT7He li",
      );
      menuEls.forEach((item) => {
        const name = item
          .querySelector("div.WV7iEb, span.fontBodyMedium")
          ?.textContent?.trim();
        const price = item
          .querySelector("div.GkFnEb, span.fontBodySmall")
          ?.textContent?.trim();
        const desc = item.querySelector("div.HlvSq")?.textContent?.trim();
        if (name)
          items.push({ name, price: price || null, description: desc || null });
      });
      return items;
    });
...
async function scrapeSinglePlace(page, placeUrl, opts = {}) {
  ...
  await page
    .waitForSelector(SELECTORS.placeName, { timeout: 15000 })
    .catch(() => {});
  ...
  if (fields.includes("menu")) {
    const overviewTab = await page.$(
      'button[aria-label*="Ringkasan"], button[aria-label*="Overview"]',
    );
    if (overviewTab) {
  ...
  if (fields.includes("email") && data.website) {
    data.email = await extractEmailFromWebsite(page, data.website, {
      scrollDelay,
      clickDelay,
    });
  }
...
export async function scrapeGoogleMaps(opts = {}) {
  ...
  if (isLinux && !hasDisplay && headless === false) {
    console.warn(
      "[Scraper] No DISPLAY detected on Linux, falling back to headless=true. Use xvfb-run for headed mode.",
    );
    useHeadless = true;
  } else {
  ...
  console.log(
    `[Scraper] Starting: "${query}" | total=${total} | fields=${fields.join(",")} | headless=${useHeadless}`,
  );
  ...
  try {
    ...
    const viewState = await Promise.race([
      page
        .waitForSelector(SELECTORS.resultsPanel, { timeout: 20000 })
        .then(() => "list"),
      page
        .waitForURL("**/maps/place/**", { timeout: 20000 })
        .then(() => "place"),
    ]).catch(() => "timeout");

    if (viewState === "place") {
      console.log(
        "[Scraper] Single result detected — scraping place directly from current page...",
      );
    ...
    if (viewState === "timeout") {
      ...
      console.log(
        "[Scraper] Results panel not found, falling back to search-box...",
      );
      ...
      await humanType(page, SELECTORS.searchBox, query, {
        minDelay: 60,
        maxDelay: 160,
      });
      ...
      if (afterSearchUrl.includes("/maps/place/")) {
        console.log(
          "[Scraper] Single result after search — scraping place directly...",
        );
        try {
    ...
    while (collectedUrls.size < total && scrollAttempts < maxScrollAttempts) {
      ...
      await page.evaluate(
        ...
        {
          selector: SELECTORS.resultsPanel,
          distance: scrollDistance + Math.random() * 100,
        },
      );
    ...
    for (let i = 0; i < urlsToScrape.length; i++) {
      ...
      console.log(
        `[Scraper] Scraping [${i + 1}/${urlsToScrape.length}]: ${url}`,
      );
